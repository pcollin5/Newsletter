...
bound_for_table <-  cbind(uethda_dummy_program, UETHDA_totals_quarter_1_2023)

names(bound_for_table) <- c("Program", "Households Served", "Individuals Served", "Service Total")

ready_for_table <- rbind(program_totals_quarter_1_2023,bound_for_table)

program_table <- table_function(ready_for_table, `Quarter 1 2023 CSBG Service Totals`) 

program_table

#### csbg board numbers ####

`Quarter 2 2023` <- quarter_2_2023

names(`Quarter 2 2023`)

names(`Quarter 2 2023`) <- c("Households", "Head of Household","Household Size", "County", "Program", "Service", "Cost of Service")
...
program_totals_quarter_2_2023 <- `Quarter 2 2023` %>%
  group_by(Program, `Head of Household`)%>%
  summarise(`Head of Household`,`Household Size`,`Total Service Cost by Household` = sum(`Cost of Service`))%>%
  unique()%>%
  ungroup()%>%
...
UETHDA_totals_quarter_2_2023 <- program_totals_quarter_2_2023 %>%
  ungroup()%>%
  select(`Households Served`, `Individuals Served`, `Service Total`)%>%
  summarise(sum(`Households Served`), sum(`Individuals Served`), sum(`Service Total`))

names(program_totals_quarter_2_2023)

uethda_dummy_program <- tibble(Program = "UETHDA Total")

uethda_dummy_program

bound_for_table_new <-  cbind(uethda_dummy_program, UETHDA_totals_quarter_2_2023)

names(bound_for_table_new) <- c("Program", "Households Served", "Individuals Served", "Service Total")

ready_for_table_new <- rbind(program_totals_quarter_2_2023,bound_for_table_new)

program_table_new <- table_function(ready_for_table_new, `Quarter 2 2023 CSBG Service Totals`) 

program_table_new

old_and_new_joined <- left_join(ready_for_table_new, ready_for_table, by = "Program")

names(old_and_new_joined) <- c("Program", "Households Served Quarter 2 2023",
                               "Individuals Served Quarter 2 2023",
                               "Service Total Quarter 2 2023",
                               "Households Served Quarter 1 2023",
                               "Individuals Served Quarter 1 2023",
                               "Service Total Quarter 1 2023")

table_function(old_and_new_joined, `CSBG Quarter 2 2023 and Quarter 1 2023 (note: only programs used from newest quarter is listed; Total includes all programs)`)
