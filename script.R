...
liheap_plot <- LIHEAP %>%
  mutate(Date = factor(Date, levels = c("Quarter 1 2022", "Quarter 2 2022", "Quarter 3 2022", "Quarter 4 2022")))%>%
  mutate(County = factor(County, levels = c("UETHDA", "Carter", "Greene", "Hancock", "Hawkins", "Johnson",
                                            "Sullivan", "Unicoi", "Washington")))%>%
  filter(Date == "Quarter 3 2022"|
           Date == "Quarter 4 2022")%>%
  ...
  ggtitle("LIHEAP Quarter 4 and Quarter 3")  
...
outcomes <- OUTCOMES %>%
  ...
  theme(legend.text = element_text(face = "bold"))+
  labs(y = " ", x = " ")+
  ggtitle("CSBG Outcomes Quarter 4 and Quarter 3")
...
outreach <- OUTREACH %>%
  ...
  labs(y = " ", x = " ")+
  ggtitle("LIHEAP Outreach Quarter 4 and Quarter 3")+
theme(plot.title = element_text(size = rel(2.25)))+
  theme(plot.title = element_text(face = "bold"))+
  theme(axis.text.x = element_text(size = rel(1.5)))+
  theme(axis.text.x = element_text(face = "bold"))+
...
wap <- WAP %>%
  ...
  theme(plot.subtitle = element_text(size = rel(1.5)))+
  theme(plot.subtitle = element_text(face = "italic"))+
  ggtitle("Weatherization Quarter 4 and Quarter 3")
...
commodities_plot <- Commodities %>%
  ...
  theme(legend.text=element_text(size=rel(1)))+
  theme(legend.text = element_text(face = "bold"))+
  ggtitle("Commodities Quarter 4 and Quarter 3")
